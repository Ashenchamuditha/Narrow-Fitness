...
export default function AdminInquiries() {
  ...
  // --- IMPROVED JSON-SAFE FETCH ---
  const fetchInquiries = async () => {
    try {
      setLoading(true);
      // We use the full URL to ensure we bypass React's dev server and hit the Backend
      const res = await fetch('http://localhost:5000/api/admin/inquiries');
      
      const contentType = res.headers.get("content-type");
      if (!res.ok || !contentType || !contentType.includes("application/json")) {
        const text = await res.text();
        console.error("❌ BACKEND ERROR: Expected JSON but got HTML/Text. Output:", text);
        return;
      }
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch('http://localhost:5000/api/admin/stats/inquiries');
      if (res.ok) {
        const data = await res.json();
        setStats(data);
      }
  ...
  const markAsRead = async (id: number) => {
    try {
      await fetch(`http://localhost:5000/api/admin/inquiries/${id}/read`, { method: 'PUT' });
  ...
  const deleteInquiry = async (id: number) => {
    ...
    try {
      await fetch(`http://localhost:5000/api/admin/inquiries/${id}`, { method: 'DELETE' });
    ...
    <AdminLayout>
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-4">
          <button 
            onClick={refreshData}
            className="p-4 bg-white border border-slate-100 rounded-2xl text-slate-400 hover:text-orange-600 transition-all shadow-sm"
          >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-[2rem] border-2 border-dashed border-slate-200 py-20 text-center">
            ...
            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">No inquiries found in this category</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <AnimatePresence>
            {filtered.map((msg) => (
              ...
              >
                {!msg.is_read && (
                  <div className="absolute top-6 right-6 flex items-center gap-1.5 px-2 py-1 bg-orange-600 text-white rounded-lg text-[8px] font-black uppercase tracking-widest animate-pulse">
                    Verified New
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                  <div className="flex items-center gap-2 text-slate-400">
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-tighter">Received: {new Date(msg.created_at).toLocaleDateString()}</span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 px-4 py-2.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg active:scale-95"
                    >
      ...
      <AnimatePresence>
        {selectedMsg && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl overflow-hidden">
               <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                  <div>
                    ...
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Verified Inquiry - Narrow Fitness</p>
                  </div>
                  <button onClick={() => setSelectedMsg(null)} className="p-2 bg-white rounded-full hover:bg-red-50 hover:text-red-500 shadow-sm transition-all"><X className="w-5 h-5" /></button>
               </div>
               
               <div className="p-8">
                  <div className="flex flex-wrap gap-x-10 gap-y-4 mb-8 pb-8 border-b border-slate-50">
                    <div>
                      <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 text-orange-600">From Athlete</p>
                      <p className="text-xs font-black text-slate-900 uppercase tracking-tight">{selectedMsg.full_name}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-900">{new Date(selectedMsg.created_at).toLocaleString()}</p>
                    </div>
                  </div>

                  <div className="bg-slate-50 p-7 rounded-3xl mb-8 min-h-[180px] border border-slate-100 shadow-inner">
                  ...
                  <div className="flex gap-3">
                    <button 
                      onClick={() => window.location.href = `mailto:${selectedMsg.email}?subject=Re: ${selectedMsg.subject} - Narrow Fitness`}
                      className="flex-1 py-4 bg-orange-600 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 hover:bg-black transition-all shadow-xl shadow-orange-200 active:scale-95"
                    >
                      <Send className="w-4 h-4" /> Compose Email Reply
                    </button>
                    <button 
